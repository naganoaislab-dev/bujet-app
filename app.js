(function () {
  ...
  function aggregateMonth(month) {
    ...
    const inactiveExpenseActual = expenseTransactions.reduce((sum, item) => {
      const category = categoryById(item.categoryId);
      return sum + (category && category.active === false && !isUnexpectedExpenseCategory(category) ? toInteger(item.amount) : 0);
    }, 0);
  ...
  function budgetedExpenseOveragesByMonth(extraExpense = null) {
    ...
    state.categories
      .filter((category) => isBudgetedExpenseCategory(category) && category.active !== false)
      .forEach((category) => {
  ...
  function projectEndForecastFromAggregates(aggregates) {
    ...
    let forecast = 0;
    const budgetedOverages = budgetedExpenseOveragesByMonth();
    ...
    aggregates.forEach((item) => {
      ...
      // Once disabled, a category's plan is excluded; its historical actual
      // expense remains a committed cost in the project-end forecast.
      forecast -= item.inactiveExpenseActual;

      // 計画内の支出は、項目ごとの過去からの持ち越しを含めた予算
      // コミットメントで相殺します。ある項目の余りで別項目の超過を
      // 見かけ上相殺しないため、各項目で相殺できない超過だけを反映します。
      item.commitmentForecastCumulative = forecast - toInteger(budgetedOverages.get(item.month));
    });

    return forecast - toInteger(budgetedOverages.get(aggregates[aggregates.length - 1].month));
  }
  ...
  function normalExpenseOverageProjection(category, sourceMonth, amount, handling = null) {
    ...
    const planAdjustmentAmount = periodMonths().slice(Math.max(0, sourceIndex)).reduce((sum, month) => (
      sum + Math.max(0, toInteger(afterScenario.deficits[month]) - toInteger(beforeScenario.deficits[month]))
    ), 0);
  ...
  function projectForecastHorizonLabel() {
    const today = appDateForNow();
    const projectEnd = parseLocalDate(state.settings.endDate);
    let months = (projectEnd.getFullYear() - today.getFullYear()) * 12 + projectEnd.getMonth() - today.getMonth();
    if (projectEnd.getDate() < today.getDate()) months -= 1;
  ...
  function entryForecastValueElement() {
    return viewHost.querySelector("[data-entry-project-forecast-value]");
  }

  function entryAnimationRect(element) {
  ...
  function entryAnimationSnapshot(categoryIds = []) {
    ...
    return {
      values,
      forecast: forecast ? {
        value: toInteger(forecast.dataset.entryAmount),
        rect: entryAnimationRect(forecast)
      } : null
    };
  }

  function queueEntryAnimation(type, targetCategoryId, sourceCategoryIds = [], details = {}, before = null) {
    pendingEntryAnimation = {
      type,
      targetCategoryId,
      sourceCategoryIds: Array.from(new Set(sourceCategoryIds.filter(Boolean))),
      details,
      before: before || entryAnimationSnapshot([targetCategoryId, ...sourceCategoryIds])
    };
  }
  ...
  function entryMoneyLayer() {
    ...
    if (!layer) {
      ...
      layer.id = "entry-money-animation-layer";
      layer.className = "entry-money-animation-layer";
  ...
  function animateEntryNumber(element, before, after, direction = "") {
    if (!element) return;
    const finalText = element.dataset.entryAnimationFinalText || element.textContent;
    delete element.dataset.entryAnimationFinalText;
    if (before === after) {
  ...
  function entryPoint(rect) {
    return rect ? {
      ...
      y: rect.top + Math.min(rect.height * 0.54, Math.max(18, rect.height - 18))
    } : null;
  }

  function flyEntryMoney(fromRect, toRect, tone = "primary", delay = 0) {
    const from = entryPoint(fromRect);
    ...
    for (let index = 0; index < particleCount; index += 1) {
      ...
      const startY = from.y + (Math.random() - 0.5) * 12;
      const endX = to.x + (Math.random() - 0.5) * 16;
      const endY = to.y + (Math.random() - 0.5) * 14;
      particle.className = `entry-money-particle is-${tone}`;
  ...
  function preparePendingEntryAnimation() {
    ...
    if (before.forecast && forecast) {
      forecast.dataset.entryAnimationFinalText = forecast.textContent;
  ...
  function playPendingEntryAnimation() {
    ...
    if (!animation || currentView !== "entry") return;
    const target = entryBudgetValueElement(animation.targetCategoryId);
    const forecast = entryForecastValueElement();
    const before = animation.before || { values: {}, forecast: null };
    const targetSnapshot = before.values[animation.targetCategoryId];
    const targetRect = entryAnimationRect(target);
    ...
    if (animation.type === "return") {
      flyEntryMoney(targetSnapshot && targetSnapshot.rect, forecastRect, "success");
      return;
    }
    if (animation.type === "new") {
      flyEntryMoney(before.forecast && before.forecast.rect, targetRect, "danger");
      return;
    }
  ...
  function renderEntry() {
    ...
    const projectForecast = projectEndForecastFromAggregates(periodMonths().map(aggregateMonth));
  ...
  function renderBudgetCards(categories, month) {
    ...
    return categories.map((category) => {
      ...
      const budgetSummary = hasNoPlan
        ...
        : `<span class="budget-card-main"><span class="budget-card-label">${remainingBudgetLabel}</span><strong data-entry-budget-value="${escapeHtml(category.id)}" data-entry-amount="${remainingBudget}" class="budget-card-amount ${budgetCardAmountSizeClass(remainingBudget)} ${remainingBudget < 0 ? "negative" : carryBudgetAvailable ? "carry-budget" : ""}">${remainingAmountLabel(remainingBudget)}</strong></span>`;
  ...
  function renderCalculatorAddBudgetCarrySources() {
    const container = document.querySelector("#calculator-add-budget-carry-sources");
    ...
    const sources = calculatorCarryFundingSources(targetCategory, sourceMonth);
  ...
  function setCalculatorFundingSourceRowState(row, toggleSelector, amountSelector, selected) {
    const toggle = row && row.querySelector(toggleSelector);
    const amountInput = row && row.querySelector(amountSelector);
    if (!row || !toggle || !amountInput) return false;
    toggle.setAttribute("aria-pressed", String(selected));
    toggle.textContent = selected ? "解除" : "使う";
    amountInput.disabled = !selected;
  ...
  function calculatorCarryFundingSelections() {
    return Array.from(document.querySelectorAll("#calculator-add-budget-carry-sources [data-carry-source-id]")).map((row) => {
      const toggle = row.querySelector("[data-carry-source-select]");
      const amountInput = row.querySelector("[data-carry-source-amount]");
      return {
        categoryId: row.dataset.carrySourceId,
        available: Math.max(0, toInteger(row.dataset.carrySourceAvailable)),
        selected: Boolean(toggle && toggle.getAttribute("aria-pressed") === "true"),
        amount: toggle && toggle.getAttribute("aria-pressed") === "true" ? Math.max(0, toInteger(amountInput.value)) : 0
      };
    });
  ...
  function calculatorCarryFundingPlanChanges(sourceMonth, funding) {
    const changes = new Map();
    const addChange = (month, amount) => changes.set(month, toInteger(changes.get(month)) + toInteger(amount));
    funding.forEach((source) => source.origins.forEach((origin) => addChange(origin.month, -origin.amount)));
    const total = funding.reduce((sum, source) => sum + source.amount, 0);
    if (total > 0) addChange(sourceMonth, total);
    return changes;
  }

  function calculatorCarryFundingPreview(sourceMonth, selections) {
    const funding = selections.map((selection) => {
      const origins = carryBudgetOriginAllocations(selection.categoryId, sourceMonth, selection.amount);
      return { ...selection, origins };
    });
    return {
      funding,
      planChanges: calculatorCarryFundingPlanChanges(sourceMonth, funding)
    };
  }
  ...
  function calculatorExternalFundingSelections(mode) {
    ...
    return Array.from(container.querySelectorAll("[data-external-source-id]")).map((row) => {
      const toggle = row.querySelector("[data-external-source-select]");
  ...
  function distributeCalculatorExternalFundingEvenly(mode) {
    ...
    const rows = Array.from(calculatorExternalFundingContainer(mode).querySelectorAll("[data-external-source-id]"))
      .filter((row) => row.querySelector("[data-external-source-select]").getAttribute("aria-pressed") === "true");
    if (amount <= 0) {
  ...
  function updateCalculatorAddBudgetMode(refresh = true) {
    ...
    document.querySelector("#calculator-add-budget-mode-select").value = mode;
    ...
    document.querySelector("#calculator-add-budget-mode-note").textContent = modeMeta.note;
    document.querySelector("#calculator-add-budget-impact-label").textContent = modeMeta.impactLabel;
  ...
  function distributeCalculatorCarryFundingEvenly() {
    const amount = Math.max(0, toInteger(document.querySelector("#calculator-add-budget-amount").value));
    const rows = Array.from(document.querySelectorAll("#calculator-add-budget-carry-sources [data-carry-source-id]")).filter((row) => row.querySelector("[data-carry-source-select]").getAttribute("aria-pressed") === "true");
    if (amount <= 0) {
      showToast("先に充当して追加する金額を入力してください");
      return;
    }
    if (!rows.length) {
      showToast("均等に配分する項目を選択してください");
      return;
    }
    const allocations = rows.map((row) => ({
      row,
    ...
    allocations.forEach((allocation) => {
      allocation.row.querySelector("[data-carry-source-amount]").value = String(allocation.amount);
    });
    updateCalculatorAddBudgetSummary();
  }
  ...
  function updateCalculatorAddBudgetSummary() {
    ...
    const carrySummary = document.querySelector("#calculator-add-budget-carry-summary");
    const sources = calculatorCarryFundingSources(category, sourceMonth);
    const selections = calculatorCarryFundingSelections().filter((selection) => selection.selected);
    const funded = selections.reduce((sum, selection) => sum + selection.amount, 0);
    const selectedCapacity = selections.reduce((sum, selection) => sum + selection.available, 0);
    const invalidSourceAmount = selections.some((selection) => selection.amount > selection.available);
  ...
  function updateCalculatorBudgetFinalReview() {
    const meta = calculatorBudgetOperationMeta();
    const summary = document.querySelector("#calculator-budget-final-review-summary");
    const warning = document.querySelector("#calculator-budget-final-review-warning");
    const reviewSummary = document.querySelector("#calculator-budget-confirmation-summary");
    summary.innerHTML = reviewSummary ? reviewSummary.innerHTML : "";
    const warnings = {
      return: "返納した予算はプロジェクト全体へ戻ります。返納後は、同じ項目へ自動では戻りません。",
      shift: "シフト元の予算が減り、選択した未来月度の予算が増えます。見込み収支の総額は変わりません。",
      new: "新しく予算を増やすため、追加額と同じ金額だけプロジェクト終了時の見込み収支が減少します。",
      carry: "持ち越し予算を今月度の計画予算へ組み込みます。見込み収支の総額は変わりません。",
      borrow: "選択した未来月度の計画予算が減り、今月度の予算が増えます。見込み収支の総額は変わりません。",
      reallocate: "選択した同月度の項目予算が減り、対象項目の予算が増えます。見込み収支の総額は変わりません。"
    };
    const warningKey = calculatorBudgetOperation === "add" ? calculatorAddBudgetMode : calculatorBudgetOperation;
    warning.textContent = warnings[warningKey] || "実行後は予算へ反映されます。";
    warning.className = `calculator-budget-final-review-warning is-${warningKey}`;
    document.querySelector("#calculator-budget-final-review-title").textContent = `${meta.completionVerb}内容の最終確認`;
  ...
  function calculatorBudgetScreenTitle() {
    if (calculatorBudgetStep === "confirm") return "内容の確認";
    if (calculatorBudgetStep === "final") return "最終確認";
    if (calculatorBudgetStep === "complete") return "完了";
    if (calculatorBudgetOperation === "return") return "返納する金額を入力";
  ...
  function updateCalculatorBudgetStageUi() {
    ...
    const finalReview = document.querySelector("#calculator-budget-final-review");
    ...
    if (!isDetail) {
      const category = calculatorContext && categoryById(calculatorContext.categoryId);
      if (category) document.querySelector("#calculator-category").textContent = category.name;
    ...
    const stepOrder = ["setup", "confirm", "final", "complete"];
    const visibleStep = calculatorBudgetProgressStep();
    const activeStepIndex = Math.max(0, stepOrder.indexOf(visibleStep));
    document.querySelectorAll("[data-budget-progress-step]").forEach((element, index) => {
      const step = element.dataset.budgetProgressStep;
      element.classList.toggle("is-current", step === visibleStep);
      element.classList.toggle("is-complete", index < activeStepIndex);
      if (step === visibleStep) element.setAttribute("aria-current", "step");
      else element.removeAttribute("aria-current");
    });
    const meta = calculatorBudgetOperationMeta();
    const progressLabel = calculatorBudgetOperation === "add" && calculatorBudgetStep === "amount"
      ? "まず追加金額を入力"
      : calculatorBudgetOperation === "add" && calculatorBudgetStep === "method"
        ? "追加方法を選択"
        : calculatorBudgetOperation === "add" && meta.detail
          ? meta.detail
          : meta.label;
    document.querySelector("#calculator-budget-progress-operation").textContent = progressLabel;
    const category = calculatorContext && categoryById(calculatorContext.categoryId);
    document.querySelector("#calculator-kind").textContent = category ? category.name : "予算調整";
    document.querySelector("#calculator-category").textContent = calculatorBudgetScreenTitle();
    if (calculatorBudgetStep === "confirm") updateCalculatorBudgetReview();
    if (calculatorBudgetStep === "final") updateCalculatorBudgetFinalReview();
  ...
  function openCalculatorBudgetReview() {
    ...
    if (calculatorBudgetOperation === "add" && calculatorBudgetStep === "amount") {
  ...
  function completeCalculatorBudgetOperation(completion) {
    const review = document.querySelector("#calculator-budget-final-review-summary");
  ...
  function setCalculatorBudgetOperation(operation = "", step = null) {
    ...
    const addMethods = document.querySelector("#calculator-add-budget-methods");
    ...
    const nextStep = step || (operation === "add" ? "amount" : isDetail ? "setup" : isMenu ? "menu" : "input");
    ...
    updateCalculatorBudgetStageUi();
    if (operation === "add" && nextStep === "amount") {
      document.querySelector("#calculator-add-budget-title").textContent = "予算を追加";
      document.querySelector("#calculator-add-budget-amount-label").textContent = "追加する予算";
      document.querySelector("#calculator-add-budget-impact-label").textContent = "追加後の状態";
      document.querySelector("#calculator-add-budget-confirm").textContent = "次へ（追加方法を選択）";
  ...
  async function runCalculatorBudgetMutation(task) {
    ...
    const confirmButtons = [
      ...
      document.querySelector("#calculator-budget-final-confirm")
    ].filter(Boolean);
  ...
  async function fundCalculatorBudgetFromExistingPlan(mode, category, sourceMonth, amount) {
    ...
    const entrySourceCategoryIds = mode === "reallocate"
      ? selections.map((selection) => sourcesById.get(selection.id).category.id)
      : [];
    const entryAnimationBefore = entryAnimationSnapshot([category.id, ...entrySourceCategoryIds]);
    const borrowedMonthPlans = mode === "borrow"
      ? selections.map((selection) => ({
        month: selection.id,
        categoryName: category.name,
        before: planAmount(category.id, selection.id),
        after: planAmount(category.id, selection.id) - selection.amount
      }))
      : [];
  ...
  async function addCalculatorBudget() {
    ...
    if (calculatorAddBudgetMode === "carry") {
      ...
      const funding = selections.map((selection) => {
        ...
        if (selection.amount > available) throw new Error(`${sourceCategory.name}から充当できる持ち越しは${formatCurrency(available)}までです`);
        const origins = carryBudgetOriginAllocations(sourceCategory.id, sourceMonth, selection.amount);
        if (origins.reduce((sum, origin) => sum + origin.amount, 0) !== selection.amount) {
          throw new Error(`${sourceCategory.name}の持ち越し元を確認できませんでした`);
        }
        return { category: sourceCategory, amount: selection.amount, origins };
      });
  ...
  document.querySelector("#calculator-budget-entry-button").addEventListener("click", () => {
    if (!calculatorContext || !calculatorContext.canShiftBudget) return;
    setCalculatorBudgetOperation("menu");
  });
  ...
  document.querySelector("#calculator-add-budget-toggle").addEventListener("click", () => {
    ...
    calculatorAddBudgetMode = "new";
    updateCalculatorAddBudgetMode(false);
    setCalculatorBudgetOperation("add", "amount");
    updateCalculatorAddBudgetSummary();
    document.querySelector("#calculator-add-budget-confirm").textContent = "次へ（追加方法を選択）";
  });
  document.querySelector("#calculator-budget-back").addEventListener("click", () => {
    if (calculatorBudgetOperation === "menu") {
      setCalculatorBudgetOperation("");
      return;
    }
    const backStep = calculatorBudgetBackStep();
    if (backStep === "menu") setCalculatorBudgetOperation("menu");
    else setCalculatorBudgetOperation(calculatorBudgetOperation, backStep);
  });
  document.querySelector("#calculator-budget-confirm-back").addEventListener("click", () => {
    setCalculatorBudgetOperation(calculatorBudgetOperation, calculatorBudgetBackStep());
  });
  document.querySelector("#calculator-budget-review-next").addEventListener("click", () => setCalculatorBudgetOperation(calculatorBudgetOperation, "final"));
  document.querySelector("#calculator-budget-final-back").addEventListener("click", () => setCalculatorBudgetOperation(calculatorBudgetOperation, "confirm"));
  ...
  document.querySelector("#calculator-add-budget-methods").addEventListener("click", (event) => {
    const method = event.target.closest("[data-add-budget-method]");
    if (!method) return;
    calculatorAddBudgetMode = method.dataset.addBudgetMethod;
    updateCalculatorAddBudgetMode();
    if (calculatorAddBudgetMode === "new") {
      setCalculatorBudgetOperation("add", "confirm");
      return;
    }
    setCalculatorBudgetOperation("add", "source");
  });
  document.querySelector("#calculator-add-budget-carry-sources").addEventListener("click", (event) => {
    const toggle = event.target.closest("[data-carry-source-select]");
    if (!toggle) return;
    const row = toggle.closest("[data-carry-source-id]");
    if (!row) return;
    setCalculatorFundingSourceRowState(
      row,
      "[data-carry-source-select]",
      "[data-carry-source-amount]",
      toggle.getAttribute("aria-pressed") !== "true"
    );
    updateCalculatorAddBudgetSummary();
  });
  document.querySelector("#calculator-add-budget-carry-sources").addEventListener("input", (event) => {
    ...
    const row = amountInput.closest("[data-carry-source-id]");
    const toggle = row && row.querySelector("[data-carry-source-select]");
    if (!row || !toggle) return;
    if (toInteger(amountInput.value) > 0 && toggle.getAttribute("aria-pressed") !== "true") {
      setCalculatorFundingSourceRowState(row, "[data-carry-source-select]", "[data-carry-source-amount]", true);
    }
    updateCalculatorAddBudgetSummary();
  });
  ...
  ["borrow", "reallocate"].forEach((mode) => {
    ...
    container.addEventListener("click", (event) => {
      ...
      setCalculatorFundingSourceRowState(
        row,
        "[data-external-source-select]",
        "[data-external-source-amount]",
        toggle.getAttribute("aria-pressed") !== "true"
      );
      updateCalculatorAddBudgetSummary();
    });
    container.addEventListener("input", (event) => {
      ...
      const toggle = row && row.querySelector("[data-external-source-select]");
      if (!row || !toggle) return;
      if (toInteger(amountInput.value) > 0 && toggle.getAttribute("aria-pressed") !== "true") {
        setCalculatorFundingSourceRowState(row, "[data-external-source-select]", "[data-external-source-amount]", true);
      }
      updateCalculatorAddBudgetSummary();
    });
